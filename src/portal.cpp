...
class AsyncVFSResponse : public AsyncAbstractResponse {
    ...
    String _filename;
public:
    AsyncVFSResponse(const String& path, const String& contentType, bool download, const String& filename) {
        _code = 200;
    ...
    ~AsyncVFSResponse() { if(_file) fclose(_file); }
...
void Portal::begin(AsyncWebServer *server, i2c_equipment_shtc3 *sensor_ptr, UserData *userdata_ptr) {
    ...
    server->on("/secret/cred.json", HTTP_GET, [](AsyncWebServerRequest *request) {
      if (UserFS.exists("/secret/cred.json")) {
          AsyncWebServerResponse *response = request->beginResponse(UserFS, "/secret/cred.json", "application/json");
          response->addHeader("X-Vault-Salt", vaultSalt);
          request->send(response);
      } else {
          // Even on 404, send the salt (useful for debugging or future stateless setup)
    ...
    server->on("/dynamic/tools.json", HTTP_POST, 
      ...
      [](AsyncWebServerRequest *request, uint8_t *data, size_t len, size_t index, size_t total) {
        ...
        if (index == 0) {
          if (!UserFS.exists("/dynamic")) UserFS.mkdir("/dynamic");
          jsonFile = UserFS.open("/dynamic/tools.json", FILE_WRITE);
        }
        if (jsonFile) {
          jsonFile.write(data, len);
          if (index + len == total) jsonFile.close();
        }
      }
    );

    server->on("/dynamic/setting.json", HTTP_POST, 
      [](AsyncWebServerRequest *request) {
          request->send(200, "application/json", "{\"status\":\"ok\"}");
      },
      NULL,
      [](AsyncWebServerRequest *request, uint8_t *data, size_t len, size_t index, size_t total) {
        ...
        if (jsonFile) {
          ...
          if (index + len == total) {
            jsonFile.close();
            loadConfig(); // Reload config to global variables after saving new settings
          }
        }
      }
    );
    ...
    for (EpyTool tool : routings) {
        server->serveStatic(tool.endpoint.c_str(), SD_MMC, tool.sdPath.c_str()).setDefaultFile("index.html");
    }

    // Index Handler
    server->on("/", HTTP_GET, [this](AsyncWebServerRequest *request) {
        request->send(SysFS, "/index.html", String(), false, std::bind(&Portal::_processor, this, std::placeholders::_1));
    });

    // API Handlers
    server->on("/system/listfiles", HTTP_GET, std::bind(&Portal::_handleListFiles, this, std::placeholders::_1));
    server->on("/system/file", HTTP_GET, std::bind(&Portal::_handleFileGetAction, this, std::placeholders::_1));
    server->on("/system/file", HTTP_DELETE, std::bind(&Portal::_handleFileDeleteAction, this, std::placeholders::_1));
    server->on("/system/mkdir", HTTP_GET, std::bind(&Portal::_handleMkdir, this, std::placeholders::_1));
    server->on("/system/reboot", HTTP_GET, std::bind(&Portal::_handleReboot, this, std::placeholders::_1));

    // Upload Handler (Requires specific lambda signature for the upload callback)
    server->on("/system/upload", HTTP_POST, 
        [](AsyncWebServerRequest *request) { 
        ...
        [this](AsyncWebServerRequest *request, String filename, size_t index, uint8_t *data, size_t len, bool final) {
            ...
            if (processedFilename.startsWith(SD_MOUNT_POINT)) {
                processedFilename = processedFilename.substring(strlen(SD_MOUNT_POINT));
            }
            this->_handleUpload(request, processedFilename, index, data, len, final);
        }
    );

    server->on("/api/secret", HTTP_POST, 
      [](AsyncWebServerRequest *request) {
          request->send(200, "application/json", "{\"status\":\"ok\"}");
      },
      NULL,
      [this](AsyncWebServerRequest *request, uint8_t *data, size_t len, size_t index, size_t total) {
          ...
          if (index + len == total) {
              ...
              if (!error && doc.containsKey("filename") && doc.containsKey("content")) {
                  ...
                  if (filename == "cred.json") {
                      ...
                      if (!UserFS.exists("/secret")) UserFS.mkdir("/secret");
  ...
  // 4. DELETE /api/secret
  server->on("/api/secret", HTTP_DELETE, [this](AsyncWebServerRequest *request) {
      if (request->hasParam("filename")) {
          ...
          if (UserFS.exists(filePath)) {
              ...
              if (filename != "cred.json") this->updateVaultIndex(filename, false);
...
String Portal::_processor(const String& var) {
    return String();
}
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        if (stat(fullPath.c_str(), &st) == 0) {
            if (S_ISDIR(st.st_mode)) {
                ...
                if (!first) json += ",";
                json += "{\"name\":\"" + fullPath + "\",\"size\":" + String((uint32_t)st.st_size) + ",\"isDir\":false}";
                first = false;
            }
        }
    }
...
void Portal::_handleFileGetAction(AsyncWebServerRequest *request) {
    if (request->hasParam("name")) {
        ...
        String shortName = (lastSlash != -1) ? fileName.substring(lastSlash + 1) : fileName;
        AsyncVFSResponse *response = new AsyncVFSResponse(fileName, "application/octet-stream", true, shortName);
        request->send(response);
    } else {
    ...
    if (request->hasParam("name")) {
        ...
        if(stat(fileName.c_str(), &st) == 0) {
            if (S_ISDIR(st.st_mode)) {
                ...
                if (remove(fileName.c_str()) == 0) request->send(200, "text/plain", "File Deleted");
...
void Portal::_handleUpload(AsyncWebServerRequest *request, String filename, size_t index, uint8_t *data, size_t len, bool final) {
    if (index == 0) {
        _uploadStartTime = millis();
        _bufferOffset = 0;
        
        if (_uploadBuffer) { free(_uploadBuffer); _uploadBuffer = NULL; }
        ...
        String folder = "/"; 
        if (request->hasParam("folder", true)) folder = request->getParam("folder", true)->value();
        else if (request->hasParam("folder")) folder = request->getParam("folder")->value();
    ...
    if (final) {
        if (_uploadFile) {
            if (_bufferOffset > 0) fwrite(_uploadBuffer, 1, _bufferOffset, _uploadFile);
            fclose(_uploadFile);
            _uploadFile = NULL;
        }
        if (_uploadBuffer) {
